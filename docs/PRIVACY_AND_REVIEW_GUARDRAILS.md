...
- 記録画面、危機モード、AI相談の回答には広告やスポンサー名を出さない。
- 将来、地域ガイドや相談先一覧など利用者が明示的に相談先を探す画面に掲載する場合は、必ず「協賛」または「PR」と明示する。
- 個人データ、記録本文、プロフィール、写真、相談内容を広告主に提供しない。提供できるのは、地域単位・分野単位に集計した数値のみ。
- AI相談のプロンプト、コンテキスト構築、回答生成では、`partners` / `sponsor_applications` / 掲載枠データを参照しない。この防火壁は恒久レビュー項目にする。

## AI相談の最小化

- 生年月日は外部AIへ送らず、送信前に「70代」などの年代へ丸める。
- 氏名、呼び名、連絡先、書類・鍵の保管場所、写真やPDFの中身は送らない。
- 相談に必要な範囲だけを送る。家族連絡先や広告/スポンサー情報はAI文脈に含めない。
