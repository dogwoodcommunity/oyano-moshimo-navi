...
- 最終説明修正と文書を含む `119af9756371dae6b8e59486d6466b6972fd6996` のCI `34001579119` は成功。
  46工程の開始sourceでのローカル実績、最終sourceのexact CI、合成復旧の独立ローカル実績を分ける。
  最終sourceのSQL10にはローカル個別再試験を含み、最終差分の全46工程連続PASSとは記録しない。
- 2026-09-06 09:59 JST頃の公開alias再照会は `dpl_GjKfchbJCDxLCyG5hrTo9oCgVDVG` / Ready。
  稼働版 `8289876` に最終説明修正は未反映。CI成功や文書更新を公開完了と扱わない。
...
- [合成復旧演習](SYNTHETIC_RECOVERY_REHEARSAL.md)を追加し、56テーブルと権限・削除証跡・合成PNGの復元PASS。
  CIには含まれない別試験。実Auth/Storage、backupより新しい削除receiptの再適用、Web/実機、
  本番RPO/RTOや本番データの復旧完了とはしない。
- 既存Pro移管プレビューは通過したが、移管・課金変更・backup取得は未実行。既存Proは2 MicroとIPv4を含み、
  今期予測41.66 USDに追加月10 USD相当という条件。新規Pro契約が必須とはせず、Free外部dumpも候補にする。
  写真別保管・保持・実backup復旧はどちらの方式でも必要。詳細は運用手順書と追記382を参照。
- 問い合わせ4値の本番設定・公開表示、本人Chrome/AAL2の5 API応答200・200・403・403・403、
  個別削除migration・receipt適用、非対話lint設定は確認済み。受信/返信、非空依頼、二者削除E2Eは別の未完了条件。
- プライバシーの保存禁止と技術的保存防止の違い、限定的な自動伏字を明確化。
  [法務最終確認メモ](LEGAL_FINAL_REVIEW_2026-09-06.md)へ未確定事項を集約。
- 実機・二者によるテスト削除・問い合わせの実受信/返信・障害の通知受信または手動認知・法務と施行日は未完了。
  正式公開の判定はNO-GOを維持し、費用/契約/本番削除の確認を飛ばさない。
- 最新差分の検証・公開証跡は `SESSION_HANDOFF.md` 追記379以降を参照。

費用・契約を変えずに進める準備：

- [ ] 運用手順書の未記入日次票へ、既存の確認手段・担当・代行・引継ぎを確定して記録する。外部alertの代わりに毎日手動確認を選べるが、実際の認知確認は省略しない。
- [ ] backup準備票で既存保管先・容量・暗号化/鍵管理・保持・実行環境・失敗確認・隔離復元先を確定する。票だけで本番backup完了としない。
- [ ] 本人が利用可能な実機・試験専用アカウントを指定し、Magic Link/MFA・二者確認を含む残試験を行う。
- [ ] Supabase管理アカウントMFAを本人が設定・確認する。移管先Team画面ではDisabledを確認済みで、アプリ側TOTPと区別する。

## 2026-09-05 終了条件の再整理

以下の日付付き実績は当時の履歴として保持する。現在の差分・公開判定は上の2026-09-06欄と最新引き継ぎを優先する。
