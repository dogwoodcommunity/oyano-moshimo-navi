...
| アカウント削除担当・代行者 | **主担当：代表取締役 池田哲也／代行者：システム責任者 池田知也** | 両名・代行者の役職と、代行者の責任範囲「主担当不在時に削除依頼の受付・本人確認・実行担当への引継ぎを代行。本番削除は登録済みapp_adminと別確認者の二者で実施」は2026-09-04に運営者確認。メールによる削除依頼は `info@bee-ch.co.jp` の共有受信箱で受けて両名へ通知する方針。アプリ内依頼は `/admin/delete-requests` のDBキューへ入り、現行実装では自動メール通知しない。実際の共有受信権限・双方通知設定・外部メール受信試験、DBキューの監視・通知方法とアプリ内テスト依頼、実行app_admin、別確認者は未確認 |
| アカウント完全削除の実行予定者 | **システム責任者 池田知也（指名方針のみ）** | 2026-09-04に運営者確認。現行の `app_admin` は削除専用ではなく全Admin API共通権限。本人確認済みの個別Supabase Auth、正確なuser ID、MFA、`app_admins` 登録、全Admin範囲の明示承認または削除専用role実装、別確認者、本番試験は未確認。指名だけでは権限を付与しない |
