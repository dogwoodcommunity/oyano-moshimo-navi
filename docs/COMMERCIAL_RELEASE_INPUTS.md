...
設定済み。新しいWebのdeploy・公開表示・受信試験は未完了（本番DBの不足を確認したためdeploy保留）。
...
| 環境変数 / 項目 | 確定する内容 | 状態 |
| --- | --- | --- |
| `LEGAL_BUSINESS_NAME` | 販売事業者の正式名称 | **株式会社BEECH（本番環境へ設定済み・新Web反映待ち）** |
| `LEGAL_RESPONSIBLE_PERSON` | 通信販売業務の責任者 | **代表取締役 池田哲也（本番環境へ設定済み・新Web反映待ち）** |
| `LEGAL_ADDRESS` | 特商法表示に用いる所在地 | **要確定** |
| `LEGAL_PHONE` | 特商法表示に用いる電話番号 | **要確定** |
| `LEGAL_PHONE_HOURS` | 電話受付時間 | **要確定** |
| `LEGAL_CONTACT` | 問い合わせメールまたはフォームURL | **info@bee-ch.co.jp（本番環境へ設定済み・新Web反映と受信確認待ち）** |
| `LEGAL_CONTACT_RESPONSE_TARGET` | 問い合わせ受付時間・一次返信目標 | **メール受付：24時間／原則3営業日以内に返信（本番環境へ設定済み・新Web反映待ち）** |
