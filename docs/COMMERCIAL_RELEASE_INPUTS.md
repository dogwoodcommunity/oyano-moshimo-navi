...
| アカウント削除担当・代行者 | **主担当：代表取締役 池田哲也／代行者：システム責任者 池田知也** | 両名・代行者の役職と、代行者の責任範囲「主担当不在時に削除依頼の受付・本人確認・実行担当への引継ぎを代行。本番削除は登録済みapp_adminと別確認者の二者で実施」は2026-09-04に運営者確認。両名の内部連絡手段、実行app_admin、別確認者は未確定 |
