...
| アカウント削除担当・代行者 | **主担当：代表取締役 池田哲也／代行者：システム責任者 池田知也** | 両名・代行者の役職と、代行者の責任範囲「主担当不在時に削除依頼の受付・本人確認・実行担当への引継ぎを代行。本番削除は登録済み削除実行者と別確認者の二者で実施」は2026-09-04に運営者確認。メールによる削除依頼は `info@bee-ch.co.jp` の共有受信箱で受けて両名へ通知する方針。アプリ内依頼は `/admin/delete-requests` のDBキューへ入り、現行実装では自動メール通知しない。実行者本人用の個別Auth招待・メール確認、削除role・pipelineの本番migration、本人端末のverified TOTP 1件・unverified 0件と設定完了時AAL2、別確認者 `代表取締役 池田哲也` の確認済みAuth・profile一致は確認済み（有効化承認は未実施）。実際の共有受信権限・双方通知設定・外部メール受信試験、DBキューの監視・通知方法とアプリ内テスト依頼、private台帳の本番適用、正確な実行者user IDの台帳記録、最小profileと無効な専用roleの同一transaction登録、承認・有効化は未確認 |
| アカウント完全削除の実行予定者 | **システム責任者 池田知也（指名方針のみ）** | 2026-09-04に運営者確認。全Admin APIへ広げない削除専用roleと本番migration、本人用個別Authのメール確認、verified TOTP 1件・unverified 0件と設定完了時AAL2、別確認者の確認済みAuth・profile一致は確認済み。ただしprivate台帳の本番適用、正確な実行者user IDの台帳記録、最小profile、`account_delete_executors` の無効行、別確認者による承認・有効化、単独テストアカウント完走は未確認。指名とMFA設定だけでは権限を付与しない |
