...
正式な利用者向け問い合わせ先は `info@bee-ch.co.jp` と確定した。ただし、`LEGAL_CONTACT` の本番設定、実際の受信・返信、公開画面の表示は未確認である。通知メール用の `NOTIFICATION_EMAIL_REPLY_TO` は別用途のため未確定のままとする。[環境変数マトリクス](ENVIRONMENT_MATRIX.md)と公開画面を照合する。
