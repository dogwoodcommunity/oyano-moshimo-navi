...
| 役割 | 氏名・連絡手段 | 代行者 |
| --- | --- | --- |
| 障害対応主責任者 / Incident Commander | **代表取締役 池田哲也**（内部連絡手段は要指定。指定後は制限付き運用台帳に記録） | **要指定** |
| リリース担当 | **要指定** | **要指定** |
...
障害対応の主責任者名は確定したが、これは全般的な運用責任者の指名や、Vercel・Supabase・GitHub・Resend・DNS等の実行権限付与を意味しない。代行者、内部連絡手段、アラート通知先とエスカレーション経路、平日・夜間・休日の当番体制、各サービスの権限とMFA・緊急時アクセス回復方法が未確定の間は、障害対応の正式運用を開始しない。秘密情報、個人電話番号、MFA、復旧コードはGitへ記録せず、制限付き運用台帳または承認済みのパスワード管理基盤で管理する。
...
これらは担当体制確定後の内部目標であり、現時点では保証ではない。主責任者名の確定だけでは30分・2時間等のack目標を実現できる体制とは扱わず、代行者、連絡経路、当番、通知先を設定して演習で確認するまで正式運用は開始しない。
