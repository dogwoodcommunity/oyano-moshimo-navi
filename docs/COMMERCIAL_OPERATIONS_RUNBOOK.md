...
正式な問い合わせ先も**要確定**である。コード上の `LEGAL_CONTACT` や `NOTIFICATION_EMAIL_REPLY_TO` が本番に設定されていること、および公開画面に正しい窓口が表示されることは未確認である。[環境変数マトリクス](ENVIRONMENT_MATRIX.md)と公開画面を照合する。
...
現行文書・画面には「正式公開前に問い合わせ先を確定」と残るため、外部設定と公開表示を確認するまでは受付準備完了としない。
