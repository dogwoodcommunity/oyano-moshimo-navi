...
| 役割 | 氏名・連絡手段 | 代行者 |
| --- | --- | --- |
| 障害対応主責任者 / Incident Commander | **代表取締役 池田哲也**（内部連絡手段は要指定。指定後は制限付き運用台帳に記録） | **池田知也**（役職・責任範囲・内部連絡手段は要指定。指定後は制限付き運用台帳に記録） |
| リリース担当 | **要指定** | **要指定** |
| Supabase・個人情報削除担当 | **代表取締役 池田哲也**（連絡手段は要指定。指定後は制限付き運用台帳に記録） | **池田知也**（役職・連絡手段は要指定。指定後は制限付き運用台帳に記録） |
| 問い合わせ一次受付 | **要指定** | **要指定** |
...
削除担当への指名だけではAdmin権限を付与しない。実行者は本人確認済みSupabase Authユーザーとして別途 `app_admins.user_id` へ登録し、削除対象本人とは別の確認者を立てる。代行者名は確定したが、役職・責任範囲・別確認者・両名の内部連絡手段・app_admin登録が未確定の間は、削除実行の正式運用を開始しない。

障害対応の主責任者と代行者の氏名は確定したが、これは全般的な運用責任者の指名や、Vercel・Supabase・GitHub・Resend・DNS等の実行権限付与を意味しない。代行者の役職・責任範囲、両名の内部連絡手段、アラート通知先とエスカレーション経路、平日・夜間・休日の当番体制、各サービスの権限とMFA・緊急時アクセス回復方法が未確定の間は、障害対応の正式運用を開始しない。秘密情報、個人電話番号、MFA、復旧コードはGitへ記録せず、制限付き運用台帳または承認済みのパスワード管理基盤で管理する。
...
これらは担当体制確定後の内部目標であり、現時点では保証ではない。主責任者・代行者の氏名確定だけでは30分・2時間等のack目標を実現できる体制とは扱わず、連絡経路、当番、通知先、各サービス権限を設定して演習で確認するまで正式運用は開始しない。
...
Incident Commanderの承認権限と、実際にVercelのrollback、SupabaseのDB操作、secret rotation等を実行できるサービス権限は別に管理する。主責任者として氏名を記録しただけでは実行権限を付与せず、実行者、権限範囲、MFA、二者確認、緊急時アクセス回復方法を制限付き運用台帳で確認してから本番操作を行う。
