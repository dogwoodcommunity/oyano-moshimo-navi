...
| アカウント完全削除の実行予定者 | **システム責任者 池田知也**（指名方針のみ。本人用個別Supabase Auth、メール確認、本人端末のTOTP・AAL2確認済み。正確なuser IDの制限付き台帳記録、最小profile、`account_delete_executors` 登録は未実施） | **代替実行者：要指定** |
| 問い合わせ一次受付 | **要指定** | **要指定** |
...
削除担当への指名だけではAdmin権限を付与しない。代行者の責任範囲「主担当不在時に削除依頼の受付・本人確認・実行担当への引継ぎを代行。本番削除は登録済み実行者と別確認者の二者で実施」は確定した。ここでいう本人確認は、利用者が `/account/delete` のMagic Link認証を完了した状態と、request ID・対象user IDの一致を確認することを指す。身分証画像、パスワード、Magic Link、access tokenは受け取らない。アカウント完全削除の実行予定者は `システム責任者 池田知也` と確定し、本人用の個別Supabase Auth招待受諾・メール確認と、本人端末でのverified TOTP 1件・現在のAAL2確認まで完了した。これは本人確認の完了だけであり、最小profile、`account_delete_executors`、Vercel・Supabase等の本番操作権限はまだ付与していない。QR、手入力用コード、6桁の数字は運営者へ送らず、正確なuser IDは制限付き運用台帳だけで扱う。削除専用roleと本番migrationは実装・適用済みで、一般Admin APIへは広がらず、緊急用管理キーも受け付けない。最小profileは監査上の本人識別子であり、それだけでは家族所属・一般Admin・削除権限を付与しない。本人確認後も、実行者とは別の確認者とその正確なprofileを確定し、無効なexecutor登録と有効化を分けて実施する。池田知也本人のアカウントを削除する場合は、別の登録済み実行者と別確認者を必要とする。メール共有受信箱と双方通知の方針は確定したが、メールとアプリ内DBキューの実際の権限・監視・通知設定・両経路の試験、実行者登録、別確認者、本番での完走試験が未確認の間は、`ACCOUNT_ERASURE_EXECUTION_ENABLED=false` を維持して削除実行の正式運用を開始しない。
...
| アカウント削除 | 本人確認済み受付、削除専用role、Bearer限定管理一覧、TOTP初回設定、実削除時AAL2、再開可能なDB証跡RPC | 実装あり。本番migrationと個別実行者のverified TOTP確認済み、実行スイッチは既定OFF。正確なuser IDの制限付き台帳記録、最小profile・role登録、別確認者、単独テストアカウント完走は外部要確認 |
...
- 実行予定者は `システム責任者 池田知也` とする方針を確定し、本人用個別Supabase Authの招待受諾・メール確認、本人端末でのverified TOTP 1件・現在のAAL2確認まで完了した。正確なuser IDの制限付き運用台帳への記録、監査用の最小profile、`account_delete_executors` の行、本番操作権限は未作成・未付与である。
...
- 本番migrationと担当者のverified TOTPは確認済みだが、環境変数の実行スイッチ、最小profile・role、別確認者、実アカウント完走を確認するまで「削除運用開始済み」と扱わない。
...
- [ ] 正確なAuth user IDを制限付き運用台帳へ記録し、最小profileと `active=false` のexecutor行を家族所属・一般Adminなしで同一transactionにより作成した。
