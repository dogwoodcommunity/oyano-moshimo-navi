...
2026-09-24実環境再確認: Supabase Free Planには自動project backupがない。AWS東京の名前照合では
もしもナビ専用の保管先は確認できず、既存のbackup stackは別サービス用。接続iPhoneは
iOS 26.6.2・Developer Mode有効だが、このMacの署名identityは0件。既存AWSテンプレートの
offline回帰PASSは実backup/隔離復元、正式署名、実機受入を意味しない。詳細は追記427。
...
| 必須 | Android 16KB内部ライブラリ | 追記417で旧判定を訂正。テスト鍵ARM64 APKの全23部品PASS、AAB一致/16KB cold起動PASS。正式成果物の全ABI/実機検査は残る |
| 必須 | サーバーと認証の整合 | 9/24: `/auth/mobile` 404、新callback未許可、初回作成RPCなし。システム認証画面への修正/配信・新binaryの実メール復帰と別人分離。通常認証には匿名/CAPTCHA有効化不要 |
| 必須 | 複数手帳のAI対象 | 選択/ID引継ぎ/状態分離/遅延応答抑止を実装・合成PASS、実機は未 |
| 必須 | AI回答のアプリ内通報 | 同意付き報告/限定管理画面/AAL2/監査/競合防止を実装・合成PASS。担当/保存期間/実受入は未 |
| 必須 | ログアウト後の通知 | 9/24: 旧登録0・v2 SQLなし。適用直前再集計、DB-first反映/保持方針/二端末実配送が必要。OFFのままでは未登録端末のログアウトも失敗する |
...
9/24はSupabase Dashboard到達とREAD ONLY SQL照会成功を確認。MFA待ちではないが、
DB変更/メール認証/実機受入は未完。具体的な判断とSolの次作業は `MOBILE_RELEASE_REVIEW_2026-09-24.md`。
