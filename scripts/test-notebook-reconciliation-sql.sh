...
for _ in $(seq 1 30); do
  if docker exec "$RECONCILIATION_CONTAINER" pg_isready -h 127.0.0.1 -U postgres >/dev/null 2>&1; then break; fi
  sleep 1
done
docker exec "$RECONCILIATION_CONTAINER" pg_isready -h 127.0.0.1 -U postgres >/dev/null
